// text ka array h jis me 9 elements h
const text = [
...
const generate = () => {
    ...
    if (isNaN(item.value) || item.value < 0 || item.value > 9) {
        ...
        dataContainer.innerHTML = `<p> ${text[randomIndex]} </p>`    //it shows only one random para every time
    }
    else {
        const local_text=shuffle(text)
        const data = local_text.slice(0, item.value)   //data a gya h yha
        //console.log(data)
        const paras = data.map( //data me se aik aik para ku utha kr return krna
            (d) => {
                return `<p> ${d} </p>`
            }
        )
        // console.log(paras)
        // console.log(paras.join("")) //array converted into string
dataContainer.innerHTML=paras.join("")   //ARRAY TO STRING and then put it into DOM 
    }
}